...
from MindMedia.DBoperations import add_post, get_new_postid, update_last_postid
...
class Create_post_dialog(Frame):
    ...
    def setup_create_post(self):
        Label(self, text="Create Post", font="monaco 30 bold", bg='pink',fg='blue').pack()
        self.main_frame = Frame(self)
        self.main_frame.pack(fill=X)
        Label(self.main_frame, text="Caption:", font=('comic sans ms',18)).grid(padx=20)
        self.caption_bar = Entry(self.main_frame, font=self.font_size)
        self.caption_bar.grid(row=0, column=1, padx=20, pady=20)
        Label(self.main_frame, text="Image upload:", font=('comic sans ms',18)).grid(row=1, column=0, padx=20, pady=20)
        self.image_btn = Button(self.main_frame, text="Browse", command=self.browse_image_path,
                                font=self.font_size,)
        self.image_btn.grid(row=1, column=1, padx=20, pady=20)

        Label(self.main_frame, text="Post type:", font=('comic sans ms',18)).grid(row=2, column=0, padx=20, pady=20)
        ...
        self.okay_btn = Button(self, text="Post", font=self.font_size, command=self.on_post,bg='pink')
    ...
    def on_post(self):
        ...
        # add_post(post_id,self.username,type,self.caption_bar.get(),0,self.image_paths)
        # using dumps to directly store  all the array as a blob data!
        data = dumps(self.image_paths)
        add_post(post_id, self.username, type, self.caption_bar.get(), 0, data)
...
# Create_post_dialog(root,"alice").pack()
# root.mainloop()
